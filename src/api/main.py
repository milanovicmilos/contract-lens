...
async def startup_event():
    ...
    classifier = HFClassifier(model_name="microsoft/deberta-v3-large") # Will lazily load or use stub if no model downloaded
    extractor = None # Optional for orchestration if not used directly
    vector_db = ChromaWrapper(persist_directory="./chroma_db", collection_name="legal_regulations")
...
async def analyze_contract(request: AnalyzeRequest, orch: ContractOrchestrator = Depends(get_orchestrator)):
    """
